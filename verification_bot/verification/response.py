...
from .verification_processor.check_similarity import get_settlment_score
from verification_bot.database import (
    whitelist_dao,
    settlement_request_dao,
    group_pairs_dao,
)
import verification_bot.database.group_messages_dao as group_messages_dao
...
class VerificationBot:
    ...
    async def initialize(self):
        ...
        if group_pairs_dao.is_source_group(self.update_message['chat']['id']):
            score = get_settlment_score(self.update_message['text'])
            
            if score > 60:
                group_messages_dao.add_group_message(self.update_message['chat']['id'], self.update_message)
    ...
    def _get_source_identifier(self, source_group_data):
        ...
        return (
            int(source_group_data['id']) or
            source_group_data.get('username')
    ...
    async def _log_settlement_request(self, response, source_group_data, group_chat_id, group_message_id):
        ...
        if response.matching_message:
            report["groupa_message_id"] = response.matching_message['message_id']
        ...
        if response.similar_messages:
            report['groupa_similar_message_ids'] = [m['message_id'] for m in response.similar_messages]
